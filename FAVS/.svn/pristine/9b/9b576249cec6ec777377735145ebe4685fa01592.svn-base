...
<head>
	...
	<script type="text/javascript">
		$(function(){
			$('#dg').datagrid({
				//请求数据的URL地址 ，远程站点  
			    url:'goods_pagingQueryJoinGtypeAndSupplier.action',
			    //在从远程站点加载数据的时候显示提示消息。
			    loadMsg: '请等待....',
			     //在请求远程数据的时候发送额外的参数。 
			    queryParams: {
			    ...
			    toolbar: [{
					...
					handler: function(){
						...
						parent.$("#win").window({
							...
							width:460,
							height:550,
							content: '<iframe src="send_goods_save.action" frameborder="0" width="100%" height="100%" />'
						});
					}
				},'-',{
					iconCls: 'icon-edit',
					...
					handler: function(){
						...
						if(rows.length!=1) {
							...
							parent.$("#win").window({
								...
								width:460,
								height:550,
								content: '<iframe src="send_goods_update.action" frameborder="0" width="100%" height="100%" />'
							});
						}
					}
				},'-',{
			    ...
			    frozenColumns:[[
			    	...
			    	{field:'gid',title:'商品编号',width:70}
			    ]],
			    //配置的列字段，field，列字段名称，与json的key捆绑
			    columns:[[  
			    	{field:'gtype.gtypename',title:'商品类别',width:70,
			        	formatter: function(value,row,index){
			        		//当前row存的就是news对象
							if(row.gtype!=null && row.gtype.gtypename!=null){
								return row.gtype.gtypename;
							} else {
								return null;
							}
						}
			        ...
			        {field:'gprice',title:'价格',width:70},
			        {field:'gunit',title:'单位',width:70},
			        {field:'gintroduce',title:'商品简介',width:300,
			        	//用来格式化当前列的值，返回的是最终的数据.当鼠标移上去的时候，把行内的数据全部显示
			        	formatter: function(value,row,index){
							return "<span title=" + value + ">" + value + "</span>";
						}
			        },
			        {field:'supplier.susername',title:'供应商',width:70,
			        	formatter: function(value,row,index){
			        		//当前row存的就是news对象
							if(row.supplier!=null && row.supplier.susername!=null){
								return row.supplier.susername;
							} else {
								return null;
							}
						}
			        },     
			        {field:'recommendation',title:'是否为推荐',width:70,
			        	formatter: function(value,row,index){
							if(value) {
