...
<head>
	...
	<script type="text/javascript">
		$(function(){
			$('#dg').datagrid({
				//请求数据的URL地址 ，远程站点  
			    url:'consumer_pagingQuery.action',
			    //在从远程站点加载数据的时候显示提示消息。
			    loadMsg: '请等待....',
			     //在请求远程数据的时候发送额外的参数。 
			    queryParams: {
			    ...
			    toolbar: [{
					...
					handler: function(){
						...
						parent.$("#win").window({
							...
							width:420,
							height:250,
							content: '<iframe src="send_consumer_save.action" frameborder="0" width="100%" height="100%" />'
						});
					}
				},'-',{
					iconCls: 'icon-edit',
					...
					handler: function(){
						...
						if(rows.length!=1) {
							...
							parent.$("#win").window({
								...
								width:420,
								height:250,
								content: '<iframe src="send_consumer_update.action" frameborder="0" width="100%" height="100%" />'
							});
						}
					}
				},'-',{
							...
							$.messager.confirm('删除确认框', '是否要删除选中的记录？', function(r){
								if (r){
								    //1.获取被选中的记录，从记录中获取相应的id
								     ...
								     //3.发送ajax请求
								     $.post("consumer_deleteByIds.action", {ids: ids}, function(result){
								     	if(result == "true"){
			    ...
			    frozenColumns:[[
			    	...
			    	{field:'cid',title:'购买者编号',width:100}
			    ]],
			    //配置的列字段，field，列字段名称，与json的key捆绑
			    columns:[[    
			        {field:'cusername',title:'购买者名称',width:100},  
			        {field:'csex',title:'性别',width:100},
			        {field:'cage',title:'年龄',width:100},
			        {field:'cphone',title:'联系方式',width:150},
