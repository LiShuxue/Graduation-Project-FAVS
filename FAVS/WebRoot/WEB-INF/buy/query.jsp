...
<head>
	...
	<script type="text/javascript">
		$(function(){
			$('#dg').datagrid({
				//请求数据的URL地址 ，远程站点  
			    url:'buy_pagingQueryJoinConsumer.action',
			    //在从远程站点加载数据的时候显示提示消息。
			    loadMsg: '请等待....',
			     //在请求远程数据的时候发送额外的参数。 
			    queryParams: {
			    ...
			    toolbar: [{
					...
					handler: function(){
						...
						parent.$("#win").window({
							...
							width:450,
							height:470,
							content: '<iframe src="send_buy_save.action" frameborder="0" width="100%" height="100%" />'
						});
					}
				},'-',{
					iconCls: 'icon-edit',
					...
					handler: function(){
						...
						if(rows.length!=1) {
							...
							parent.$("#win").window({
								...
								width:450,
								height:470,
								content: '<iframe src="send_buy_update.action" frameborder="0" width="100%" height="100%" />'
							});
						}
					}
				},'-',{
							...
							$.messager.confirm('删除确认框', '是否要删除选中的记录？', function(r){
								if (r){
								    //1.获取被选中的记录，从记录中获取相应的id
								     ...
								     //3.发送ajax请求
								     $.post("buy_deleteByIds.action", {ids: ids}, function(result){
								     	if(result == "true"){
			    ...
			    frozenColumns:[[
			    	...
			    	{field:'bid',title:'求购信息编号',width:100}
			    ]],
			    //配置的列字段，field，列字段名称，与json的key捆绑
			    columns:[[    
			        {field:'btitle',title:'标题',width:100},  
			        {field:'bcontent',title:'内容',width:350,
			        	//用来格式化当前列的值，返回的是最终的数据.当鼠标移上去的时候，把行内的数据全部显示
			        	formatter: function(value,row,index){
							return "<span title=" + value + ">" + value + "</span>";
						}
			        },   
			        {field:'btime',title:'发布时间',width:100},
			        {field:'consumer.cusername',title:'发布人',width:100,
			        	formatter: function(value,row,index){
			        		//当前row存的就是buy对象
							if(row.consumer!=null && row.consumer.cusername!=null){
								return row.consumer.cusername;
							} else {
								return null;
							}
						}
