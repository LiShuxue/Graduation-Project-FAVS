		...
		$(function(){
			$('#dg').datagrid({
			    ...
			    toolbar: [{
					...
					handler: function(){
						...
						parent.$("#win").window({
							...
							width:360,
							height:140,
							content: '<iframe src="send_gtype_save.action" frameborder="0" width="100%" height="100%" />'
						});
					}
				},'-',{
					iconCls: 'icon-edit',
					...
					handler: function(){
						...
						if(rows.length!=1) {
							...
							parent.$("#win").window({
								...
								width:300,
								height:140,
								content: '<iframe src="send_gtype_update.action" frameborder="0" width="100%" height="100%" />'
							});
						}
					}
				},'-',{
							...
							$.messager.confirm('删除确认框', '是否要删除选中的记录？', function(r){
								if (r){
								    //1.获取被选中的记录，从记录中获取相应的id
								     ...
								     //3.发送ajax请求
								     $.post("gtype_deleteByIds.action", {ids: ids}, function(result){
								     	if(result == "true"){
			    ...
			    frozenColumns:[[
			    	...
			    	{field:'gtypeid',title:'类别编号',width:100}
			    ]],
			    //配置的列字段，field，列字段名称，与json的key捆绑
			    columns:[[    
			        {field:'gtypename',title:'类别名称',width:100,
